...
public class CitationClassBuilder implements WikiComponentBuilder
{
    ...
    public List<DocumentReference> getDocumentReferences()
    {
        ...
        try {
            Query query = queryManager.createQuery(
                "SELECT doc.space, doc.name FROM Document doc, doc.object(" + Constants.SPACE + "." + Citation.CLASS
    ...
    public List<WikiComponent> buildComponents(DocumentReference reference) throws WikiComponentException
    {
        ...
        DocumentReference documentReference =
            new DocumentReference(getXWikiContext().getDatabase(), Constants.SPACE, Citation.CLASS + "Class");
        ...
        try {
            ...
            if (!rightService.hasAccessLevel("admin", doc.getAuthor(), "XWiki.XWikiPreferences", getXWikiContext())) {
                throw new WikiComponentException(String.format(
                    "Failed to building " + Citation.CLASS + "Class components from document "
            ...
            for (final BaseObject obj : objects) {
                ...
                cit.setPrettyId(
                    doc.getDocumentReference().getLastSpaceReference().getName() + "." + doc.getDocumentReference()
                        .getName() + "." + obj.getStringValue("key") + " (" + obj.getGuid() + ")");
            ...
            throw new WikiComponentException(
                String.format("Failed to build " + Citation.CLASS + "Class components from document [%s]",
