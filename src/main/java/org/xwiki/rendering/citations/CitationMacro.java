...
public class CitationMacro extends AbstractMacro<CitationMacroParameters>
{
     ...
     *
     * @param key key of the citation object
     ...
     *
     * @param value String Value which is evaluated
    ...
    @Override
    public List<Block> execute(CitationMacroParameters parameters, String content, MacroTransformationContext context)
        throws MacroExecutionException
    {
        ...
        if (citation != null) {
            ...
            if (citation.getType().equals("Conference") || citation.getType().equals("Inbook") || citation.getType()
                .equals("Incollection") || citation.getType().equals("Inproceedings")
                || citation.getType().equals("Manual") || citation.getType().equals("Masterthesis")
                || citation.getType().equals("Misc") || citation.getType().equals("Phdthesis")
                || citation.getType().equals("Proceedings") || citation.getType().equals("Unpublished")
                || citation.getType().equals("Techreport"))
            {
                String value = citation.getAuthor();
                if (isValid(value)) {
                    citationContent.append(value);
        ...
        List<Block> result =
            this.contentParser.parse(citationContent.toString(), context, true, context.isInline()).getChildren();

        /** Add pages at the end of the parsed content */
        if (hasPages) {
            RawBlock rb =
                new RawBlock("<sup><span class=\"footnoteRef\">, p." + citePages + "</span></sup>", Syntax.HTML_4_01);
            result.add(rb);
        }
